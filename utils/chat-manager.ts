...
export class ChatManager {
  ...
  async sendChatMessage(message: string, tabId?: number): Promise<string> {
    backgroundLogger.info('ChatManager.sendChatMessage called', { message: message.substring(0, 50), tabId });
    ...
    try {
      ...
      backgroundLogger.debug('📤 Messages for API', { messageCount: messagesForAPI.length, messages: messagesForAPI.map(m => ({ role: m.role, contentType: typeof m.content })) });
      ...
      const onComplete = async (text: string, _toolCalls?: any[], _toolResults?: any[], uiMessage?: any) => {
        ...
        if (uiMessage && uiMessage.parts) {
          ...
          backgroundLogger.debug('Using AI SDK UI message parts', { partsCount: uiMessage.parts.length });
        } else {
      ...
      backgroundLogger.error('Exception in ChatManager.sendChatMessage', { error: error instanceof Error ? error.message : error });
      ...
      try {
        ...
        backgroundLogger.error('Error updating storage with error message', { error: storageError });
      }
  ...
  getServiceInfo() {
    return {
      ...
      availableTools: ['screenshot', 'find', 'extract', 'summary', 'click', 'type', 'scroll', 'wait'],
